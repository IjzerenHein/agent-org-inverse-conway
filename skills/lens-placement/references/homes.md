...
Reviewer column: "owning team" means the team that owns the path (`ownership_paths` in the profile), under ordinary review (a lens). "Harness owner" means a wall, as the paragraph under procedure step 11 of `SKILL.md` defines it: propose it, never apply it.

Every ledger entry in the table is one new file, written from the template this skill ships for its kind. The Default file column names that template.
...
|---|---|---|---|---|---|
| 1 | Findings entry, status `hypothesis` | `docs/ledgers/findings/YYYY-MM-DD-slug.md`, from `assets/finding-entry-template.md` | owning team | Never loads as a rule (V7). A spec may pin it as a source. | Stays `hypothesis` until a human adjudicates it. Then place the adjudicated statement again from step 1. |
...
| 5 | Attested line | `docs/ledgers/facts/YYYY-MM-DD-slug.md`, from `assets/fact-entry-template.md`: the claim, with `evidence: quote` and the verbatim text, or `evidence: attestation` with the person, role, `valid-from` and `valid-until` (V7) | the attesting person; owning team | Pinned as a source in the spec. It counts only at status `recorded` or `attested` and inside its dates; a `draft` is a hypothesis. | At `valid-until` it is `expired`: re-attest or void. |
...
| 5 | Experiment record or dated finding | `docs/ledgers/experiments/YYYY-MM-DD-slug.md`, from `assets/experiment-entry-template.md` with `type: product`, or `docs/ledgers/findings/YYYY-MM-DD-slug.md`, from `assets/finding-entry-template.md` | owning team | Pinned as a source in the spec | An experiment's expiry (the latest of `extensions[]`, else `expiry`) forces keep or kill at the retro, and a kill creates a `retire` item (learning loop). A finding carries its date. |
| 5 | Open question | `docs/ledgers/questions/YYYY-MM-DD-slug.md`, from `assets/question-entry-template.md` | the human who can answer (`can-answer`) | Does not load | Open until its three `closed-` fields are filled, by the rule at the template's `closed-by` field (section 1, artefact chain). |
...
| 8 | Obligations entry, plus a date- or version-keyed failing check | `docs/ledgers/obligations/YYYY-MM-DD-slug.md`, from `assets/obligation-entry-template.md`; the check with the project's checks | owning team; the check as in step 2 | The check starts failing at the date or version in `due`. | Closed when the commitment is met; the check goes with it. |
